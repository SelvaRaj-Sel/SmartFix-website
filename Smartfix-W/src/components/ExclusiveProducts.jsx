...
const products = [
  {
    ...
    applications: "Machine control, process automation, safety systems, and plant-wide control architectures.",
    specifications: ["CompactLogix and ControlLogix", "GuardLogix safety control", "SIMATIC S7-1200 and S7-1500", "Scalable I/O and networking"],
  },
  {
    id: 2,
    ...
    images: [PLC2],
    applications: "Operator stations, production monitoring, machine visualization, and HMI upgrades.",
    specifications: ["PanelView Plus and PanelView 5000", "Siemens Comfort Panels", "Touchscreen operator interfaces", "Alarm and recipe management"],
  },
  {
    id: 3,
    icon: Gauge,
    ...
    images: [PLC1],
    applications: "Pumps, conveyors, fans, compressors, mixers, and other variable-speed motor applications.",
    specifications: ["PowerFlex 523 and 755", "SINAMICS G120, V20, and V30", "Energy-efficient speed control", "Motor protection and diagnostics"],
  },
  {
    id: 4,
    icon: Zap,
    ...
    images: [PLC3],
   applications: "Packaging, robotics, material handling, and precision machine motion.",
    specifications: ["Kinetix 5500 and 5700", "SIMOTICS servo motors", "High-speed positioning", "Coordinated multi-axis motion"],
  },
  {
    id: 5,
    icon: ScanEye,
    ...
    images: [PLC4],
    applications: "Defect detection, measurement, identification, alignment, and automated quality inspection.",
    specifications: ["Industrial cameras and sensors", "Barcode and code readers", "Vision-based inspection", "Quality data and traceability"],
  },
  {
    id: 6,
    icon: Cable,
    ...
    images: [PLC1],
    applications: "Field measurement, machine connectivity, industrial networking, and equipment safety.",
    specifications: ["Industrial sensors and transmitters", "Remote I/O modules", "Communication adapters", "Network switches and safety relays"],
  },
];
...
const ProductCard = ({
  ...
  return (
    <motion.article
      ...
      whileHover={{ y: -4 }}
      className={`group relative mx-auto w-full max-w-[420px] cursor-pointer overflow-hidden rounded-2xl border bg-(--dark2) shadow-[0_10px_30px_rgba(2,6,23,0.18)] transition-all duration-500 ease-out hover:border-cyan-300/30 hover:shadow-[0_20px_45px_rgba(0,160,210,0.18)] ${
        isSelected ? "border-cyan-300/60 ring-1 ring-cyan-300/30" : "border-white/10"
      }`}
    >
      <div className="relative h-[160px] w-full overflow-hidden bg-[#071827]">
        {images.length > 0 ? (
          <AnimatePresence mode="wait">
            <motion.img
              ...
              animate={{ x: 0, opacity: 1, scale: isHovered ? 1.08 : 1 }}
              exit={{ x: -80, opacity: 0 }}
              transition={{
      ...
      <div className="relative p-4">
        <div className="flex items-center gap-2.5">
          <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg border border-cyan-400/20 bg-cyan-400/5 text-(--primary) transition-all duration-500 group-hover:border-cyan-300/40 group-hover:bg-cyan-400/10 group-hover:scale-105">
            <Icon size={19} className="transition-transform duration-500 group-hover:scale-110" />
          </div>
  ...
  return (
    <AnimatePresence mode="wait">
      <motion.article
        ...
        className="mt-8 overflow-hidden rounded-2xl border border-cyan-300/25 bg-[#071827] shadow-[0_18px_45px_rgba(0,160,210,0.12)]"
        aria-live="polite"
      >
        <div className="grid gap-6 p-5 sm:p-7 lg:grid-cols-[0.7fr_1.3fr] lg:p-8">
          ...
          <div>
            <p className="text-[0.68rem] font-bold uppercase tracking-[0.2em] text-(--primary)">Selected Product</p>
            <h3 className="mt-1 text-2xl font-semibold text-white">{product.title}</h3>
            <p className="mt-4 text-sm leading-7 text-slate-300">{product.description}</p>
            <p className="mt-3 text-sm leading-7 text-slate-400">
              <span className="font-semibold text-slate-200">Best suited for: </span>
              {product.applications}
            </p>
            <h4 className="mt-5 text-sm font-bold uppercase tracking-[0.18em] text-(--primary)">Specifications</h4>
            <ul className="mt-4 grid gap-3 sm:grid-cols-2">
              {product.specifications.map((specification) => (
                <li key={specification} className="rounded-lg border border-white/10 bg-white/5 px-4 py-3 text-sm text-slate-200">
                  {specification}
                </li>
              ))}
            </ul>
    ...
    <div className="relative mx-auto max-w-8xl px-5 sm:px-8 lg:px-12">
      ...
      >
        ...
        <p className="max-w-xl text-base leading-7 text-slate-300 lg:text-lg">
          ...
          <span className="text-(--rockwell-red) hover:text-red-500"> Rockwell </span>
          and
          <span className="text-(--siemens-teal) hover:text-cyan-500"> Siemens.</span>
        </p>
      </motion.header>
      ...
      <div className="relative mx-auto max-w-8xl px-7 sm:px-9">
        ...
        >
          {products.map((product, index) => (
            <div key={product.id} className="w-full shrink-0 snap-start md:w-[calc((100%_-_1rem)/2)] lg:w-[calc((100%_-_2.5rem)/3)]">
              <ProductCard
                {...product}
